...
def run_all_scenarios (truths, reps, dir='../data'): 
    ...
    all_names = []
    for i in range(scenarios): 
        run_on_cluster(dir='%s/v%s' % (dir, i), true_cf = truths[i][0], true_std = truths[i][1], std_bias = truths[i][2], reps=reps, tag=str(i))
        all_names.append('cc%s_comb' % (i))
        
    hold_string = '-hold_jid %s ' % ','.join(all_names)
    call = 'qsub -cwd %s -N cc_compile cluster_shell.sh cluster_compile.py %i "%s"' % (hold_string, scenarios, dir)
    subprocess.call(call, shell=True)
